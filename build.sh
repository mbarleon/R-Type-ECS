...
function _configure_and_build() {
    ...
    _info "building shared library r-type_ecs with Ninja..."
    if ninja -j"$(_cpus)" r-type_ecs; then
        _success "built shared library r-type_ecs"
        exit 0
    fi
    _error "build error" "failed to build r-type_ecs"
}
...
function _fclean()
{
    ...
    rm -rf \
        ...
        unit_tests plugins code_coverage.txt \
        unit_tests-*.profraw unit_tests.profdata \
        vgcore* cmake-build-debug
}
...
do
    case $args in
        -h|--help)
            cat << EOF
USAGE:
      $0    builds r-type_ecs project (Ninja)
