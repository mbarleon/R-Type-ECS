...
function Invoke-ConfigureAndBuild([string]$BuildType, [string[]]$ExtraFlags) {
    ...
    try {
        ...
        Info "building shared library r-type_ecs with Ninja"
        & ninja -j (Get-CpuCount) r-type_ecs | Out-Host

        Ok "built shared library r-type_ecs"
    } catch {
        Fail "build error" "failed to build r-type_ecs"
    } finally {
        Pop-Location
    }
}
...
@"
USAGE:
  .\build.ps1       builds r-type_ecs project (Ninja)
